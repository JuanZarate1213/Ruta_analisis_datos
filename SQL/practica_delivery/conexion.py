...
print(df_u[['nombre', 'ciudad']]) #4. Selecciona las columnas nombre y ciudad de la tabla usuarios.
...
df_p = df_only_p.rename(columns={'monto_total': 'valor_factura'}) #9. Cambia el nombre de la columna monto_total a valor_factura (.rename()).
...
df_p['monto_con_propina'] = df_only_p['monto_total'] * 1.10 #12. Crea una nueva columna llamada monto_con_propina que sea el monto_total + 10%.
...
df_p[(df_u['ciudad'] == 'Bogotá') & (df_only_p['monto_total'] > 30000)] #18. Filtra pedidos que sean de 'Bogotá' Y que el monto sea mayor a 30,000.
print(df_only_p['monto_total'].round(0)) #19. Redondea todos los valores de monto_total a 0 decimales.
df_u['nombre'].str.upper() #20. Convierte todos los nombres de los clientes a MAYÚSCULAS (.str.upper()).


###BLOQUE 3###
df_pedidos_usuarios = pd.merge(df_only_p, df_u, on='id_usuario')
print(df_pedidos_usuarios.groupby('ciudad')['monto_total'].mean().round(2)) #21. Calcula el promedio de monto_total por cada ciudad (.groupby()).

df_union = pd.merge(df_only_p, df_res, on='id_restaurante')
top_ventas = df_union.groupby('nombre_r')['monto_total'].sum().sort_values(ascending=False).head(1)
print(top_ventas) #22. ¿Cuál es el restaurante que más dinero ha facturado en total?
...
df_monto_x_res= pd.merge(df_only_p, df_res , on='id_restaurante')
...
print(df_only_p.sort_values(by='fecha_pedido', ascending=False).head(10)) #26. Ordena el DataFrame por fecha_pedido (de más reciente a antiguo).

df_p_res= pd.merge(df_only_p, df_res , on='id_restaurante')
df_top_especialidad_ciudad= pd.merge(df_p_res, df_u, on='id_usuario')
print(df_top_especialidad_ciudad.groupby(['ciudad','especialidad'])['monto_total'].size().sort_values(ascending=False)) #27. Agrupa por ciudad y especialidad para ver qué se pide más.
...
df_maestro.to_excel('reporte_final1.xlsx', index=False, sheet_name='Pedidos_2026') #40. Exporta el mismo resultado a un archivo de Excel (.xlsx).
...
# Usando el pivot_table que ya hiciste (Punto 35)
# Sumamos todas las especialidades por ciudad para tener el total
ventas_por_ciudad = pivot_pedidos.sum(axis=1)
...
# Usamos el df_maestro que creamos en los pasos anteriores
grafico = sns.countplot(data=df_maestro, x='ciudad', palette='viridis', hue='ciudad', legend=False)

# Personalización
plt.title('Distribución de Pedidos por Ciudad', fontsize=15)
...
sns.histplot(df_only_p['tiempo_entrega_min'], bins=20, kde=True, color='orange')
